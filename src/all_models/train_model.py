...
def train_model(train_set, dev_set):
    ...
    初始化模型，优化器和损失函数。训练过程可以在enity和event之间相互切换，并且在training set上完成聚类操作。
...
def train_and_merge(clusters, other_clusters, model, optimizer, loss, device,
    ...
    with torch.no_grad():
        ...
        merge(clusters, cluster_pairs, other_clusters, model, device,
              topic.docs, epoch, topics_counter, topics_num, threshold,
              is_event, config_dict["use_args_feats"],
